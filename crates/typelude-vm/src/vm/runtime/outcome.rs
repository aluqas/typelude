...
/// Trapped execution with the pre-step state of the failing instruction and both trace streams.
#[derive(Debug)]
pub struct Raised<Trap, State, Trace>(pub PhantomData<(Trap, State, Trace)>);

/// Suspended execution with the advanced state committed before yielding and both trace streams.
#[derive(Debug)]
