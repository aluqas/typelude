...
/// Associates a host call signature with the value type pushed back onto the VM stack on resume.
pub trait HostSignature {
    type Response;
}

/// A suspended host request carrying the host signature, the current argument stack, and the
/// response contract required to resume execution.
#[derive(Debug)]
