...
use typelude_vm::{
    opcode::{OpAdd, OpGetLocal, OpHostCall, OpLet, OpLt, OpPush, OpSetLocal, OpWhile},
    vm::run::composed::ComposedVmState,
};
use typenum::{U0, U1, U2, U10, U55, U89, U151, U152};

use crate::support::{
    ComposedRun, ComposedTraceLen, EmptyComposedState, OutcomeRequest, OutcomeState, RequestSig,
...
type FibFinalState = ComposedVmState<
    tyarray![ELit<U55>],
...
fn fibonacci_10_composed_final_state_is_stable() {
    type ComposedOut = ComposedRun<EmptyComposedState, FibProgram10>;
...
fn fibonacci_10_composed_trace_length_is_stable() {
    ...
    assert_type_eq_all!(ComposedTraceLen<ComposedOut>, U151);
}

#[test]
fn fibonacci_10_then_host_suspend_has_stable_state_and_request() {
    type ComposedOut = ComposedRun<EmptyComposedState, FibProgram10WithHost>;
    type ComposedState = <ComposedOut as OutcomeState>::Output;
    ...
    assert_type_eq_all!(ComposedTraceLen<ComposedOut>, U152);
}
